...
public class JdbcForEmployeeDao implements Dao<Employee> {

    private DataSource dataSource;

    public JdbcForEmployeeDao(DataSource dataSource) {
    ...
    public Employee create(Employee newInstance) throws PersistentException {
        String sql = "INSERT INTO app.entries (name, surname, telephone_number, employee_status, login, password, department, branch_office) VALUES (?,?,?,?,?,?,?,?);";
    ...
    public void update(Employee transientObject) throws PersistentException {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = getPreparedStatement(connection, "UPDATE app.entries SET name = ?, address = ?, telephone_number = ?, employee_status = ?, login = ?, password = ?, department = ?, branch_office = ?  WHERE id = ?;", 4, transientObject.getEmployeeID())) {
    ...
    public void delete(Employee persistentObject) throws PersistentException {
        try (Connection connection = dataSource.getConnection();
             PreparedStatement statement = getPreparedStatement(connection, "DELETE FROM app.entries WHERE id = ?;", 1, persistentObject.getEmployeeID())) {
            statement.execute();
        } catch (SQLException e) {
            throw new PersistentException(e);
        }
